...
public class DollarRatesWithPataShouldNotBeShownSteps {
	@Page
	FirstScreenPage firstScreen;

	public void validateDollarRatesWithPata(String nit) {
		...
		List<List<String>> listOfRatesWithPata = ratesQueries.getAllTheDollarRatesWithPata(nit, "Tasas Dolar sin PATA");

		String[] listDollarRates = ratesQueries.getRatesWithBalance(nit, "USD", "Tasas Dolar sin PATA");

		int numberOfDollarRatesInTheBackendWithPata = listOfRatesWithPata.size();
		int numberDollarRates = listDollarRates.length;

		int totalFeesThatShouldBeSeenInTheFront = numberDollarRates - numberOfDollarRatesInTheBackendWithPata;

		assertTrue("No está coincidiendo la cantidad de datos que hay en el backend con las que hay en el frontend",
				numberOfDollarRatesInTheFrontend == totalFeesThatShouldBeSeenInTheFront);
	}
}
